...
import type { ExecFn, NotifyFn, TerminalOpenerDeps } from "./terminal-opener.js";

function buildExec(responses: Record<string, { stdout: string; stderr?: string }>): ExecFn {
...
function buildFailingExec(failPatterns: string[]): ExecFn {
  return async (cmd: string, args: string[]) => {
    ...
    for (const pattern of failPatterns) {
      if (key.includes(pattern)) {
        const error = new Error(`Command failed: ${key}`) as Error & { stderr: string };
...
function buildDeps(overrides?: {
  ...
  failPatterns?: string[];
}): TerminalOpenerDeps & { notifications: Array<{ title: string; body: string }> } {
...
describe("formatTarget", () => {
  it("formats target back to string", () => {
    expect(formatTarget({ session: "main", window: "1", pane: "0" })).toBe("main:1.0");
  });

  it("formats non-numeric window", () => {
    expect(formatTarget({ session: "work", window: "dev", pane: "2" })).toBe("work:dev.2");
  });
});
...
describe("validatePane", () => {
  it("returns true when tmux display-message succeeds", async () => {
    ...
    const result = await validatePane({ session: "main", window: "1", pane: "0" }, exec);
...
describe("switchClient", () => {
  it("returns ok:true on success", async () => {
    ...
    const result = await switchClient("/dev/ttys001", { session: "main", window: "1", pane: "0" }, exec);
  ...
  it("passes args as array (Fix #6 prevention)", async () => {
    ...
    await switchClient("/dev/ttys001", { session: "main", window: "2", pane: "1" }, exec);
...
describe("openTerminal (full flow)", () => {
  it("succeeds with valid target, single client, and terminal detected", async () => {
    ...
    const exec: ExecFn = async (cmd, args) => {
      ...
      if (key.includes("list-clients")) return { stdout: "/dev/ttys001\n", stderr: "" };
    ...
    const notifications: Array<{ title: string; body: string }> = [];
    const notify: NotifyFn = (title, body) => notifications.push({ title, body });
  ...
  it("returns switch-failed when switch-client errors", async () => {
    const exec: ExecFn = async (cmd, args) => {
      if (args.includes("display-message")) return { stdout: "", stderr: "" };
      if (args.includes("list-clients")) return { stdout: "/dev/ttys001\n", stderr: "" };
