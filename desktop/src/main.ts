...
app.whenReady().then(async () => {
  ...
  if (!serverStarted) {
    ...
    const trayManager = createTrayManager({
      windowManager,
      configManager,
      onQuit: () => shutdown(server, configManager, soundManager, trayManager, windowManager),
    });
    trayManager.create();
    return;
  }
 ...
 */
function registerGlobalShortcut(windowManager: { toggleVisibility(): void }): void {
 ...
 */
function registerConfigIPC(configManager: ReturnType<typeof createConfigManager>): void {
  ipcMain.handle("pf:get-config", () => {
    return configManager.get();
  });

  ipcMain.handle("pf:set-config", (_event, payload: { key: string; value: unknown }) => {
...
function wireSessionSoundAlerts(
  ...
  instance.registry.onEvent((event) => {
    if (event.type === "session:added") {
      soundManager.handleStateChange(event.session.sessionId, event.session.activity);
    }
