...
vi.mock("electron", () => ({
	dialog: {
		...
		showErrorBox: (...args: unknown[]) =>
			mockDialog.showErrorBox(...(args as [unknown, unknown])),
	},
}));
...
describe("createEmbeddedServer", () => {
	...
	it("start() shows port conflict dialog on EADDRINUSE", async () => {
		mockServerInstance.start.mockRejectedValueOnce(
			new Error(
				"Failed to start server on 127.0.0.1:8314: listen EADDRINUSE: address already in use",
			),
		);
		// User clicks "Quit"
	...
	it("start() shows generic error for non-port failures", async () => {
		mockServerInstance.start.mockRejectedValue(
