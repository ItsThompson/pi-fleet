...
export function loadConfig(configPath?: string): PiFleetConfig {
  ...
  try {
    ...
    return {
      version: 1,
      preferences: {
        ghostMode: parsed.preferences?.ghostMode ?? DEFAULT_CONFIG.preferences.ghostMode,
        ghostOpacity: parsed.preferences?.ghostOpacity ?? DEFAULT_CONFIG.preferences.ghostOpacity,
        soundEnabled: parsed.preferences?.soundEnabled ?? DEFAULT_CONFIG.preferences.soundEnabled,
      },
    };
  } catch {
