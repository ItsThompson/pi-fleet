...
interface NotificationDismissStore {
  ...
  dismissAll: (entries: Array<{ sessionId: string; stateChangedAt: string }>) => void;
...
export const useNotificationDismissStore = create<NotificationDismissStore>((set, get) => ({
  dismissed: new Map(),
  ...
  dismissAll: (entries) => {
    set((state) => {
      ...
      entries.forEach((entry) => {
        next.set(entry.sessionId, { dismissedStateChangedAt: entry.stateChangedAt });
      });
      return { dismissed: next };
    });
  },
