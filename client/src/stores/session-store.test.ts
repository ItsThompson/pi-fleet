...
describe("session-store", () => {
  ...
  it("updates a session", () => {
    useSessionStore.getState().addSession(buildSession());
    useSessionStore.getState().updateSession(buildSession({ activity: "idle", model: "Claude Sonnet 4" }));
  ...
  it("updates activityChangedAt when activity changes", () => {
    useSessionStore.getState().addSession(buildSession({ activity: "processing" }));
    const firstChanged = useSessionStore.getState().activityChangedAt.get("session-1")!;

    // Same activity: should not change timestamp
    useSessionStore.getState().updateSession(buildSession({ activity: "processing", model: "new model" }));
    const sameChanged = useSessionStore.getState().activityChangedAt.get("session-1")!;
    expect(sameChanged).toBe(firstChanged);

    // Different activity: should update timestamp
    useSessionStore.getState().updateSession(buildSession({ activity: "idle" }));
    const newChanged = useSessionStore.getState().activityChangedAt.get("session-1")!;
