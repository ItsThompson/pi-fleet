...
describe("filter-store", () => {
	...
	describe("passesFilter", () => {
		...
		it("uses OR logic with multiple filters", () => {
			...
			expect(useFilterStore.getState().passesFilter(approvalSession)).toBe(
				true,
			);
			expect(useFilterStore.getState().passesFilter(processingSession)).toBe(
				false,
			);
		});
	});
	...
	describe("podPassesFilter", () => {
		it("passes all pods when no filters active", () => {
			...
			expect(useFilterStore.getState().podPassesFilter(pod, sessions)).toBe(
				true,
			);
		});
		...
		it("passes pod when any member matches filter", () => {
			...
			const sessions = new Map([
				[
					"session-1",
					buildSession({ sessionId: "session-1", activity: "processing" }),
				],
				[
					"session-2",
					buildSession({ sessionId: "session-2", activity: "idle" }),
				],
			]);
		...
		it("handles missing sessions gracefully", () => {
			...
			const sessions = new Map([
				[
					"session-1",
					buildSession({ sessionId: "session-1", activity: "idle" }),
				],
			]);
