...
describe("notification-dismiss-store", () => {
	...
	describe("dismiss", () => {
		it("adds a session to the dismissed map", () => {
			useNotificationDismissStore
				.getState()
				.dismiss("session-1", "2025-01-01T00:01:00Z");

			const { dismissed } = useNotificationDismissStore.getState();
			expect(dismissed.has("session-1")).toBe(true);
			expect(dismissed.get("session-1")?.dismissedStateChangedAt).toBe(
	...
	describe("dismissAll", () => {
		it("dismisses all provided entries", () => {
			...
			expect(dismissed.get("session-2")?.dismissedStateChangedAt).toBe(
				"2025-01-01T00:02:00Z",
			);
			expect(dismissed.get("session-3")?.dismissedStateChangedAt).toBe(
		...
		it("preserves existing dismissals not in the provided entries", () => {
			useNotificationDismissStore
				.getState()
				.dismiss("session-existing", "2025-01-01T00:00:00Z");
	...
	describe("isDismissed", () => {
		it("returns false when session was never dismissed", () => {
			const result = useNotificationDismissStore
				.getState()
				.isDismissed("session-1", "2025-01-01T00:01:00Z");
		...
		it("returns true when currentStateChangedAt is older than dismissedStateChangedAt", () => {
			useNotificationDismissStore
				.getState()
				.dismiss("session-1", "2025-01-01T00:05:00Z");
		...
		it("returns false when currentStateChangedAt is newer (session cycled: re-notification)", () => {
			...
			const result = useNotificationDismissStore
				.getState()
				.isDismissed("session-1", "2025-01-01T00:05:00Z");
			expect(result).toBe(false);
		});
		...
		it("handles re-notification correctly after full cycle", () => {
			...
			expect(
				useNotificationDismissStore
					.getState()
					.isDismissed("session-1", "2025-01-01T00:10:00Z"),
			).toBe(false);
		});
	});
});
