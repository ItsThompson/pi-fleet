...
describe("useFilteredPods", () => {
  ...
  it("filters pods by single active filter", () => {
    ...
    const pods = [
      ...
      buildPod({ leadSessionId: "b", state: "processing", memberSessionIds: ["s2"] }),
  ...
  it("filters pods by multiple active filters", () => {
    ...
    useSessionStore.setState({ sessions });
    useFilterStore.setState({ activeFilters: new Set(["idle", "pending_approval"]) });

    const pods = [
      buildPod({ leadSessionId: "a", state: "idle", memberSessionIds: ["s1"] }),
      buildPod({ leadSessionId: "b", state: "pending_approval", memberSessionIds: ["s2"] }),
      buildPod({ leadSessionId: "c", state: "processing", memberSessionIds: ["s3"] }),
    ];
