...
function buildCluster(overrides?: Partial<ClusterDefinition>): ClusterDefinition {
  return {
    id: "cluster-1",
    name: "My Cluster",
...
describe("sse-dispatcher", () => {
  ...
  describe("session events", () => {
    ...
    it("routes session:updated to updateSession", () => {
      ...
      expect(useSessionStore.getState().sessions.get("s1")!.activity).toBe("idle");
    });
    ...
    it("routes session:removed to removeSession", () => {
      ...
      dispatcher.dispatch("session:removed", JSON.stringify({ sessionId: "s1" }));
  ...
  describe("pod events", () => {
    ...
    it("routes pod:dissolved to removePod", () => {
      ...
      dispatcher.dispatch("pod:dissolved", JSON.stringify({ leadSessionId: "lead-1" }));
  ...
  describe("cluster events", () => {
    ...
    it("routes cluster:deleted to removeCluster", () => {
      ...
      dispatcher.dispatch("cluster:deleted", JSON.stringify({ clusterId: "cluster-1" }));

      expect(useClusterStore.getState().clusters).toHaveLength(0);
    });

    it("routes cluster:reordered to reorderClusters", () => {
      useClusterStore.getState().addCluster(buildCluster({ id: "a", sortOrder: 0 }));
      useClusterStore.getState().addCluster(buildCluster({ id: "b", sortOrder: 1 }));
      const dispatcher = createSSEDispatcher(deps);

      dispatcher.dispatch("cluster:reordered", JSON.stringify({ orderedIds: ["b", "a"] }));
  ...
  describe("connected event", () => {
    it("calls deps.onConnected()", () => {
      ...
      dispatcher.dispatch("connected", JSON.stringify({ serverTime: "2025-01-01T00:00:00Z" }));
  ...
  describe("invalid event shape (Zod failure)", () => {
    it("logs event type and issues when session data is invalid", () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ type: "session:added", issues: expect.any(Array) }),
      );
      expect(useSessionStore.getState().sessions.size).toBe(0);
    });
    ...
    it("logs event type and issues when pod data is invalid", () => {
      ...
      dispatcher.dispatch("pod:formed", JSON.stringify({ leadSessionId: null }));
