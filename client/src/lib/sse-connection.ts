...
function calculateBackoffDelay(attemptCount: number): number {
  return Math.min(INITIAL_BACKOFF_MS * Math.pow(2, attemptCount - 1), MAX_BACKOFF_MS);
}
...
export function createSSEConnection(config: SSEConnectionConfig): SSEConnection {
  ...
  function connect(): void {
    ...
    setState({ connected: false, reconnecting: state.attemptCount > 0, attemptCount: state.attemptCount });
    ...
    es.onerror = () => {
      ...
      setState({ connected: false, reconnecting: true, attemptCount: nextAttempt });
  ...
  function close(): void {
    closed = true;
    cleanup();
    setState({ connected: false, reconnecting: false, attemptCount: state.attemptCount });
  }
