...
export async function refetchAllState(baseUrl: string): Promise<void> {
  try {
    const [sessionsRes, podsRes] = await Promise.all([
      fetch(`${baseUrl}/api/sessions`).then((r) => (r.ok ? r.json() : { sessions: [] })),
      fetch(`${baseUrl}/api/pods`).then((r) => (r.ok ? r.json() : { pods: [] })),
    ]);

    useSessionStore.getState().setSessions((sessionsRes.sessions ?? []) as RegisteredSession[]);
