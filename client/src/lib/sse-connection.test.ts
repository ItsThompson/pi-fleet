...
import { createSSEConnection, type ConnectionState, type SSEConnection } from "./sse-connection";
...
describe("sse-connection", () => {
  ...
  function createConnection(overrides?: Partial<Parameters<typeof createSSEConnection>[0]>) {
    connection = createSSEConnection({
      url: "http://localhost:8314/api/events",
      eventTypes: ["session:added", "session:updated", "heartbeat", "connected"],
      onEvent,
      onStateChange,
      ...overrides,
    });
  ...
  describe("event routing", () => {
    it("routes named events to onEvent callback with type and raw data", () => {
      ...
      expect(onEvent).toHaveBeenCalledWith("session:added", '{"sessionId":"abc"}');
    });
    ...
    it("does not route events after close()", () => {
      ...
      expect(onEvent).not.toHaveBeenCalledWith("session:added", expect.anything());
    });
  });
  ...
  describe("onStateChange fires on lifecycle transitions", () => {
    ...
    it("fires on successful open", () => {
      ...
      expect(onStateChange).toHaveBeenCalledWith(
        expect.objectContaining({ connected: true, reconnecting: false, attemptCount: 0 }),
      );
    });
    ...
    it("fires on error/reconnecting", () => {
      ...
      expect(onStateChange).toHaveBeenCalledWith(
        expect.objectContaining({ connected: false, reconnecting: true, attemptCount: 1 }),
      );
    });
