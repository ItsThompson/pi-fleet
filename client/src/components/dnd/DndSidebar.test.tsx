...
describe("Sidebar DnD Integration", () => {
	...
	it("renders pods as draggable elements within cluster sections", () => {
		...
		const podButton = screen
			.getByText("api-service")
			.closest("[role='button'][aria-roledescription='draggable']");
	...
	it("unclustered section is NOT sortable", () => {
		...
		const sortableButtons = screen
			.getAllByRole("button")
			.filter((btn) => btn.getAttribute("aria-roledescription") === "sortable");
	...
	it("pods in unclustered section are draggable", () => {
		const pods = new Map([
			[
				"lead-1",
				buildPod({ leadSessionId: "lead-1", displayName: "orphan-session" }),
			],
		]);
		usePodStore.setState({ pods });
		useClusterStore.setState({
	...
	it("both clusters and unclustered are drop zones (have droppable containers)", () => {
		const pods = new Map([
			...
			[
				"lead-2",
				buildPod({ leadSessionId: "lead-2", displayName: "project-b" }),
			],
		]);
		usePodStore.setState({ pods });
		useClusterStore.setState({
	...
	it("pod click still navigates (DnD does not interfere with click)", async () => {
		const pods = new Map([
			[
				"lead-1",
				buildPod({ leadSessionId: "lead-1", displayName: "clickable-pod" }),
			],
		]);
		usePodStore.setState({ pods });
		useClusterStore.setState({
