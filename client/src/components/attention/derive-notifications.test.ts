...
describe("deriveNotificationEntries", () => {
  ...
  it("returns entries only for pending_approval and idle sessions", () => {
    const sessions = new Map([
      ["s1", buildSession({ sessionId: "s1", activity: "pending_approval", agentName: "agent-a" })],
      ["s2", buildSession({ sessionId: "s2", activity: "idle", agentName: "agent-b" })],
      ["s3", buildSession({ sessionId: "s3", activity: "processing", agentName: "agent-c" })],
      ["s4", buildSession({ sessionId: "s4", activity: "running_tool", agentName: "agent-d" })],
    ]);
    const pods = new Map<string, Pod>();
  ...
  it("uses pod displayName for podDisplayName field", () => {
    const sessions = new Map([
      ["s1", buildSession({ sessionId: "s1", activity: "idle", agentName: "my-agent" })],
    ]);
    const pods = new Map([
      ["lead-1", buildPod({ leadSessionId: "lead-1", memberSessionIds: ["s1"], displayName: "My Pod" })],
    ]);
  ...
  it("uses agentName for sessionName, falls back to cwd directory", () => {
    const sessions = new Map([
      ["s1", buildSession({ sessionId: "s1", activity: "idle", agentName: "named-agent" })],
      ["s2", buildSession({ sessionId: "s2", activity: "idle", agentName: undefined, cwd: "/home/user/my-project" })],
    ]);
    const pods = new Map<string, Pod>();
    const activityChangedAt = new Map([
  ...
  it("falls back to lastSeen when activityChangedAt missing", () => {
    const sessions = new Map([
      ["s1", buildSession({ sessionId: "s1", activity: "idle", lastSeen: "2025-01-01T00:10:00Z" })],
    ]);
    const pods = new Map<string, Pod>();
