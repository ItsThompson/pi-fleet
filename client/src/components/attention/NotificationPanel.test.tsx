...
describe("NotificationPanel", () => {
	...
	it("renders empty state when no sessions need attention", () => {
		...
		useSessionStore.setState({
			sessions,
			activityChangedAt: new Map([["session-1", "2025-01-01T00:00:00Z"]]),
		});
	...
	it("renders entries in reverse-chronological order", () => {
		...
		const newerIndex = entries.findIndex(
			(el) => el.textContent === "newer-session",
		);
		const olderIndex = entries.findIndex(
			(el) => el.textContent === "older-session",
		);
	...
	describe("dismiss notifications", () => {
		...
		it("removes all notifications when Clear all is clicked", () => {
			...
			expect(screen.getByText("Notifications (0)")).toBeInTheDocument();
			expect(
				screen.getByText("No sessions need attention"),
			).toBeInTheDocument();
		});
