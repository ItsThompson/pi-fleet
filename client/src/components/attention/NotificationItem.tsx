...
export function NotificationItem({ entry, onOpen, onDismiss }: NotificationItemProps) {
  return (
    <div className="rounded-md border p-3 space-y-1">
      <div className="flex items-center gap-2">
        <SessionStatusDot status={entry.state} />
        <span className="text-sm font-medium truncate flex-1">{entry.sessionName}</span>
        <Button
          variant="ghost"
          size="xs"
