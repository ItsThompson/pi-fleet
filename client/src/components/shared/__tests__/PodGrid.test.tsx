...
describe("PodGrid", () => {
	it("renders sections with titles and items", () => {
		...
		render(
			<PodGrid sections={sections} hasActiveFilters={false} totalCount={3} />,
		);
	...
	it("hides sections with no items", () => {
		...
		render(
			<PodGrid sections={sections} hasActiveFilters={false} totalCount={2} />,
		);
	...
	it("uses default empty messages when custom ones are not provided", () => {
		...
		expect(
			screen.getByText("No items match the active filters."),
		).toBeInTheDocument();
	});
	...
	it("applies responsive grid classes to item containers", () => {
		...
		const { container } = render(
			<PodGrid sections={sections} hasActiveFilters={false} totalCount={1} />,
		);
