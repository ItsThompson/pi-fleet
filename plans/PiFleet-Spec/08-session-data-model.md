...
function createActivityTracker(deps: ActivityTrackerDeps) {
  ...
  return {
    get current(): ActivityStatus { return current; },

    onTurnStart(): void { transition("processing"); },
    onTurnEnd(): void { transition("idle"); },
    onToolExecutionStart(): void { transition("running_tool"); },
    onToolExecutionEnd(): void { transition("processing"); },
    onToolPermissionRequested(): void { transition("pending_approval"); },
    onToolPermissionGranted(): void { transition("running_tool"); },
    onToolPermissionDenied(): void { transition("processing"); },
  };
}
```
...
if (payload.contextUsage !== undefined) existing.contextUsage = payload.contextUsage;
if (payload.turnCount !== undefined) existing.turnCount = payload.turnCount;
if (payload.thinkingLevel !== undefined) existing.thinkingLevel = payload.thinkingLevel;
if (payload.lastToolName !== undefined) existing.lastToolName = payload.lastToolName;
```
