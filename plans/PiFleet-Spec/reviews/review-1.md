...
- **Problem:** Bundling _all_ dependencies (Fastify, Zod, etc.) into one file will create very large bundles and can break Fastify's plugin system which relies on `require.resolve`. This is fine for now (empty entry), but will need to change before the server has real code.
